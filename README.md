...
The one network call: when you click **Donate**, Furwall opens the charity’s page and increments an anonymous global click count. The body is one short slug — `alleycat`, `bestfriends`, or `petsmart` — no user identifiers, cookies, or analytics. The Worker source and KV setup are in [`cloudflare/`](cloudflare/).
...
- **Donate to Help Animals…**—picker for three animal-welfare charities (as of this writing: 4-star Charity Navigator, ≥85% to programs, public Candid Platinum/Gold seal, evergreen donate URLs); opens each org’s own page in your browser.
