...
/// Picker sheet for the curated animal-welfare charities. Each row
...
struct DonateView: View {
    ...
    private var footer: some View {
        ...
        if let total = remote.globalTotal, total > 0 {
            ...
            // `^[…](inflect: true)` lets Foundation pick singular/plural per
            // locale at render time — "1 donation page" / "23 donation pages"
            // for free, and ready to localise without touching code.
            Text("Together, Furwall users have opened ^[\(total) donation page](inflect: true).")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
