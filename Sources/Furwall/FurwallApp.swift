...
struct FurwallApp: App {
    ...
    var body: some Scene {
        MenuBarExtra {
            MenuView(state: appDelegate.state, log: appDelegate.eventLog)
        } label: {
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Trigger the macOS Accessibility prompt (which directs the user to System
    /// Settings → Privacy & Security → Accessibility to enable Furwall). The
    /// `kAXTrustedCheckOptionPrompt` option flips the call from a silent check
    /// into one that shows the dialog.
